...
ここでは「プライベートアドレス」「ループバックアドレス」のような、使い道を切り口にした種類について触れます。
...
:::message
教科書的には「全世界で一意」という観点で「グローバルアドレス」と表現することが多いのですが、AWS では「インターネットに直接接続可能な（公開の）IP アドレス」という意味を込めて「パブリック IP アドレス」と表現しています。
:::
...
があり、AWS ではそれぞれ以下のケースに対応（相当）します。

- パブリックサブネットで EC2 に Elastic IP アドレスを割り当てた場合
  - 静的 NAT で固定のグローバルアドレスが割り当てられる
- パブリックサブネットで EC2 に対してパブリック IP アドレスの割り当てを有効にした場合
  - 一種の動的 NAT（※）でグローバルアドレスが割り当てられる
    - （※）EC2 が起動している間は割り当てたグローバルアドレス（パブリック IP アドレス）がプールに戻されず同じアドレスの割り当てが持続する
...
- IPv4 アドレスは全 32 ビットで 8 ビットずつ「`.`」で区切って表記
  - クラスレスのアドレス表記では、アドレスの後に「`/`」で区切ってプレフィックス長を記す
- プライベートアドレス・グローバルアドレスなどの種類がある
  - 組織内で自由に割り当てて使う／RIR などから割り当てを受けて使う（インターネットに直接接続可能）／その他の特定の目的に使う
- IPv4 アドレスは固定的に割り当てるか、DHCP を使って動的に割り当てる
- NAT はプライベートアドレスをグローバルアドレスに変換して、プライベートアドレスしか持たないホストからのインターネットアクセスを可能にする技術
  - 静的 NAT・動的 NAT・NAPT があるが、AWS の NAT ゲートウェイは NAPT
