...
- サーバー用 VPC「sv-ipv4-to-dualstack-vpc」画面 → **「アクション」** → **「CIDR の編集」** から
...
- ALB「ipv4-to-dualstack」画面 → **「アクション」** → **「セキュリティグループの編集」** から
...
- ALB「ipv4-to-dualstack」画面 → **「アクション」** → **「IP アドレスタイプの編集」** から
...
つまり、あるホストから IPv4 と IPv6 の両方のホストと直接通信しようとする場合は、IPv4 と IPv6 の両方が動作する状態に構成する必要があります。

このような構成や仕組みのことを一般に「デュアルスタック」と呼びます。
...
なお一般的にはサブネットのサイズを`/64`に設定し、前半 64 ビットをサブネットプレフィックス、後半 64 ビットをインターフェース識別子（IID）と呼びます。

ただしこのままだと表記が長くなりすぎるので、省略形の表記ルールがあります。

- 16 ビット区切りのフィールドの先頭で連続する`0`は省略可
- 複数の 16 ビットフィールドで`0`が連続する場合は「`::`」で省略可（1 か所のみ）

などです（先ほどの例は`2001:db8::2:0:1`または`2001:db8:0:0:0:2::1`のように省略表記が可能）。

### IPv6 グローバルユニキャストアドレス（GUA）について

IPv6 は IPv4 と同じく用途別に複数種のアドレスがあります。IPv4 におけるグローバルアドレスと同様、全世界で一意に割り当てられたアドレスをグローバルユニキャストアドレス（GUA）と呼びます。

IPv6 にも、IPv4 におけるプライベートアドレスと同様の使い方が可能なアドレス（ULA：ユニークローカルユニキャストアドレス）がありますが、IPv6 ULA と GUA の間で NAT を行うことは非推奨です。AWS の NAT ゲートウェイでもサポートされていません。

そのため、通常はサブネットのパブリック・プライベートに関係なく、インターネットへのアクセスが必要であれば GUA を割り当てるのが一般的です。
...
例えば IP アドレスを文字列として扱う場合、最大文字数が IPv4 の場合サブネット長なしで 15 文字、サブネット長付きで 18 文字なのに対し、それぞれ 39 文字・43 文字に増えます。
...
そして、IP アドレスを文字列として扱うケースでアドレス同士の比較が必要な場合、IPv4 アドレス以上に IPv6 アドレスの表記が柔軟であり 1 つのアドレスの表記方法が多数存在しうるため、これを一意に表記するか、省略なしの表記にする必要があります。
...
- Web サーバー自体には IPv6 アドレスを割り当てなくても良いが、アプリケーション側で IPv6 アドレスを正しく扱えるようにしておく
