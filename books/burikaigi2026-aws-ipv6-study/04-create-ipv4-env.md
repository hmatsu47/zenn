...
インターネットから ALB に HTTP アクセスすると、Web サーバーがアクセス元 IP アドレスを示す`HTTP_X_FORWARDED_FOR`を返すように構成しています。
...
:::message
このセッションでは実施しませんが、独自ドメインで ALB を公開する場合、Route 53 ホストゾーンに ALB 向けの A レコード（エイリアス）を登録します。
:::
...
マネジメントコンソールから EC2 に接続して`curl`コマンドを実行し、サーバー環境やインターネットへの接続テストを行うための環境です。
