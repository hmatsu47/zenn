---
title: "クライアント環境の IPv6 対応（デュアルスタック編）"
---
...
- SLAAC : サブネットプレフィックスなどをルータから受信し、インターフェース識別子（IID）を自動生成する
  - IID を Modified EUI-64 形式（MAC アドレスを変換）で生成するのが標準だったが、MAC アドレスが露出するプライバシー問題があるので現在は非推奨
  - Modified EUI-64 形式の代わりに、一時的な IID をランダムに生成して一定時間で切り替えていく Temporary ID 方式が使われることが多い
...
IPv4・IPv6 へのアクセスを同時に試行しますが、実質的に IPv6 が優先される形になります。
...
- クライアント環境をデュアルスタック化すると、クライアント自身が IPv4・IPv6 の通信を使い分ける形になる
- IPv4 は NAT ゲートウェイへ、IPv6 は Egress Only インターネットゲートウェイに流す形でルーティングする
- Egress Only インターネットゲートウェイは IPv6 to IPv6 の NAT が使えない状況で IPv4 における NAT ゲートウェイと同等の使い勝手を提供する
