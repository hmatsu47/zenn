...
:::message
特に近年は「IPv4 からの移行方法」のひとつとして「IPv6-Mostly Network」が注目されていますので、興味がある方は調べてみてください。

AWS の環境では「IPv6-Mostly Network」の手法は（今のところ）そのまま採用することは難しいですが、オンプレ環境の移行で役立つ場面がありそうです。

**【参考】**

- https://blog.nic.ad.jp/2025/11130/
...
- 各 EC2 インスタンスを削除
- （オプションまで進めた場合は）AMI を登録解除・EBS スナップショットを削除
- ALB（ロードバランサー）を削除
- 同・ターゲットグループを削除
- NAT ゲートウェイを削除
- Elastic IP アドレスを解放
- VPC エンドポイント（EC2 インスタンス接続用）を削除
- VPC を削除
