...
- ロードバランサーから「ipv4-to-dualstack」を開く
- リスナーとルールの「HTTP:80」を開く → 「アクション」 → 「リスナーの編集」

![](/images/burikaigi2026-aws-ipv6-study/035001-modify-alb-tg-ipv6.png)

- ターゲットグループ : ipv6-server
- **「変更内容の保存」** をクリック

### `curl`コマンドまたは IPv6 アドレスが使えるブラウザから接続テスト

EC2（クライアント）に接続し、サーバー環境の ALB に`curl`コマンドでアクセスするか、手元の環境が IPv6 対応であればブラウザからアクセスして接続テストを行います。

#### ブラウザからの接続テスト実行例

![](/images/burikaigi2026-aws-ipv6-study/036001-check-alb-tg-ipv6.png)

アクセス元の IPv6 アドレスが表示されたら成功です。
...
すでに何度か触れていますが、IPv6 では GUA と ULA の NAT は非推奨で、AWS の NAT ゲートウェイでもサポートしていません。つまり、何らかのプロキシや、アドレス変換（NAT66 / NPTv6）が可能なアプライアンス（後述）を立てるなどしないと、IPv6（ULA）専用サブネットからインターネットにアクセスすることはできません。
...
先に示した手順の冒頭で EC2 の AMI を作成してその AMI を使ってインスタンスを作成したのも、この「RPM リポジトリにアクセスできない」ことが理由です。

初期状態の構築では、起動（作成）時のユーザーデータで nginx と PHP を`dnf install`しているのですが、これは IPv6（ULA）専用サブネットではリポジトリにアクセスできないので正しく実行できません。

AWS の各種サービスのエンドポイントにアクセスする際も VPC に個別の IPv6（プライベート）エンドポイントを作成する必要があり、Egress Only インターネットゲートウェイでのエンドポイントアクセスと比べて料金面で不利になります。
