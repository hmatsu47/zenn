...
| パラメータ名             | 説明                                                                                                                                                                                                                | デフォルト値                       |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| `storage_layout`         | `memory_optimized` : 統計的バイナリ量子化を使用<br>`plain` : 非圧縮                                                                                                                                                 | `memory_optimized`                 |
| `num_neighbors`          | ノードあたりの最大隣接数を指定<br>値を大きくすると精度が上がる一方、グラフの探索が遅くなる                                                                                                                          | 50                                 |
| `search_list_size`       | インデックス作成時の貪欲探索アルゴリズムで使用される S パラメーター<br>値が大きいほどインデックス作成が遅くなる一方でグラフの品質が向上する                                                                         | 100                                |
| `max_alpha`              | アルゴリズムのアルファパラメーター<br>値を大きくするとグラフの品質が向上するが、代償としてインデックス作成が遅くなる                                                                                                | 1.2                                |
| `num_dimensions`         | インデックスの次元数<br>デフォルト値（0）ではすべての次元をインデックス化する<br>その他の数値を指定するとサブベクトルを切り出して[マトリョーシカ埋め込み](https://huggingface.co/blog/matryoshka)に使うことができる | 0（すべての次元）                  |
| `num_bits_per_dimension` | 統計的バイナリ量子化使用時の次元あたりビット数                                                                                                                                                                      | 2（900 次元未満）<br>1（それ以上） |

- インデックス検索時のパラメーター

| パラメータ名                     | 説明                                                | デフォルト値 |
| -------------------------------- | --------------------------------------------------- | ------------ |
| `diskann.query_search_list_size` | グラフ探索時に考慮する追加候補の数                  | 100          |
| `diskann.query_rescore`          | 再スコアリング（※）要素数（0 : 再スコアリング無効） | 50           |

※再スコアリング：最小の近似距離を持つ`query_rescore`個の要素を取り、正確な距離で再スコアリングして正確な距離で最も近いものを返す処理。
