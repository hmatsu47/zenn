...
以下の SQL 文で生成しました。全体で 26 GiB 近くあります。
...
- **`test_long_uk`のデータページを読み出す途中でバッファプールがいっぱいになった**
...
ただし、先に挙げたテストデータ生成 SQL 文ではデータ生成時に`TEXT`の長さをランダムにしている関係で **`test_long`（テーブル）全体のサイズがバッファプールのサイズを超えてしまう** ことがあります。そのようなケースでは、2 回目以降は 1 回目より速くはなるものの毎回ストレージからの読み出しが必要になるため、 **トータルで MySQL 8.0 より遅くなる可能性** があります。

その場合でも、 **`test_long_uk`については主キーのクラスタインデックス（＝テーブルデータ本体）よりはるかに小さいセカンダリインデックスをスキャンするだけで済む** ため、`SELECT COUNT(*)`の実行順に関わらず **`test_short`とほぼ同等の所要時間** になります。
:::
